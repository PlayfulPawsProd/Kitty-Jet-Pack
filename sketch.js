...
// Code for my Master~♥ Nyaa~! (Update Button Added!)
// PART 1 of 2 - Added a refresh button for updates! Nya~!
...
let updateButtonColor; // NEW: Update Button Color
...
let updateButton; // NEW: Update Button variable
...
function setup() {
  try {
    ...
    updateButtonColor = color(100, 100, 200, 180); // Bluish update button
...
// --- Define Button Bounds --- (Added Update Button)
function defineButtonBounds() {
    if (width && height) {
        ...
        let updateButtonW = width * 0.4; let updateButtonH = height * 0.05; // Size for update button
        updateButton = { x: 15, y: height - updateButtonH - 10, w: updateButtonW, h: updateButtonH }; // Position bottom-left
...
// windowResized (Updates ALL button bounds)
function windowResized() { /* ... same canvas resize ... */ let aspectRatio = windowHeight / windowWidth; let internalCanvasHeight = floor(internalCanvasWidth * aspectRatio); internalCanvasHeight = max(1, internalCanvasHeight); resizeCanvas(internalCanvasWidth, internalCanvasHeight); console.log(`Canvas resized to internal resolution: ${width}x${height}`); if (kitty) { kitty.baseY = height - 80; kitty.y = kitty.baseY; kitty.size = min(width, height) * 0.08; kitty.x = constrain(kitty.x, kitty.size / 2, width - kitty.size / 2); } else { console.warn("windowResized: kitty not ready yet."); } initializeBackgroundElements(); earthY = height * 1.5; defineButtonBounds(); } // Call defineButtonBounds which now sets all buttons
...
// END OF PART 1
// PART 2 of 2 - Update Button Added! Nya~!

// Start Screen - ADDED Update Button
function displayStartScreen() {
    ...
    if (updateButton) {
        rectMode(CORNER);
        fill(updateButtonColor);
        noStroke();
        rect(updateButton.x, updateButton.y, updateButton.w, updateButton.h, 5);
        fill(textColor);
        stroke(textStrokeColor);
        strokeWeight(1);
        textSize(updateButtonTextSize);
        textAlign(CENTER, CENTER);
        text("Check for Updates", updateButton.x + updateButton.w / 2, updateButton.y + updateButton.h / 2);
    }
...
// --- Input Handling --- (Check Update Button) ---
function handlePressStart() {
    ...
    if (gameState === 'start') {
        // Update Button Check
        if (updateButton && pressX >= updateButton.x && pressX <= updateButton.x + updateButton.w && pressY >= updateButton.y && pressY <= updateButton.y + updateButton.h) {
             console.log("Update button pressed! Reloading...");
             window.location.reload(true); // Force reload
             return; // Stop processing
        }
        // Endless Button Check

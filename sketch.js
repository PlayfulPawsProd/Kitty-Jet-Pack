...
// PART 1 of 2 - Calling store functions, drawing bow! Nya~!

let kitty;
let plushies = [];
let score = 0; let highScore = 0; let endlessHighScore = 0;
let currentStreak = 0; let highestStreakInSession = 0;
let totalPlushiesCollected = 0; // NEW: Grand total for store
let lives = 3;
let gameState = 'intro'; // intro, start, playing, store, gameOverCutscene, gameOver
...
let backButtonColor;
let storeButtonColor, storeButtonTextColor; // Moved from store.js for consistency
let textStrokeColor;
...
function setup() {
  try {
    ...
    storeButtonColor = color(150, 150, 220, 180); storeButtonTextColor = color(255); // Store button color
    ...
    // Define button bounds (will also call setupStoreLayout)
    defineButtonBounds();

    // Load purchases from store.js
    if (typeof loadItems === 'function') { loadItems(); }
    else { console.error("!!! CRITICAL: loadItems() function not found!"); }
...
// --- Define Button Bounds --- (Includes Store button)
function defineButtonBounds() {
    if (width && height) {
        ...
        storeButton = { x: width / 2 - storeButtonW / 2, y: endlessModeButton.y + endlessButtonH * 1.8, w: storeButtonW, h: storeButtonH }; // Position Store button
        let backButtonSize = min(width, height) * 0.1;
        backButton = { x: width - backButtonSize - 15, y: 15, w: backButtonSize, h: backButtonSize * 0.6 };

        // Call store layout setup
        if (typeof setupStoreLayout === 'function') { setupStoreLayout(width, height); }
        else { console.error("setupStoreLayout() function not found!"); }

    } else { console.warn("defineButtonBounds called too early."); endlessModeButton = undefined; storeButton = undefined; backButton = undefined; }
}
...
// Game Over Screen (UPDATED for Endless High Score)
...
function displayHUD() { /* ... same ... */ let hudTextSize = min(width, height) * 0.04; let heartSize = hudTextSize * 1.3; let backTextSize = hudTextSize * 0.9; let streakTextSize = hudTextSize * 1.1; let currentHudTextColor = hudTextColorLight; if (visualStage === maxVisualStageIndex && isEndlessMode) { currentHudTextColor = hudTextColorDark; } fill(currentHudTextColor); stroke(textStrokeColor); strokeWeight(1.5); textSize(hudTextSize); textAlign(LEFT, TOP); let lineY = 15; let lineSpacing = hudTextSize * 1.3; text(`Plushies: ${score}`, 15, lineY); lineY += lineSpacing; text(`Difficulty: ${difficultyStage + 1}`, 15, lineY); lineY += lineSpacing; if (isEndlessMode) { fill(endlessModeTextColorOff); textSize(hudTextSize); text(`Best Run: `, 15, lineY); fill(endlessModeTextColorOn); textSize(streakTextSize); text(`${highestStreakInSession}`, 15 + textWidth("Best Run: "), lineY); lineY += lineSpacing; fill(endlessModeTextColorOn); textSize(streakTextSize); text(`Streak: ${currentStreak}`, 15, lineY); } else { textAlign(RIGHT, TOP); let hearts = ''; for (let i = 0; i < lives; i++) { hearts += '♥ '; } fill(heartColor); textSize(heartSize); noStroke(); text(hearts, width - 15, 10); } if (gameState === 'playing' && isEndlessMode && backButton) { rectMode(CORNER); fill(backButtonColor); noStroke(); rect(backButton.x, backButton.y, backButton.w, backButton.h, 3); fill(textColor); stroke(textStrokeColor); strokeWeight(1.5); textSize(backTextSize); textAlign(CENTER, CENTER); text("Back", backButton.x + backButton.w / 2, backButton.y + backButton.h / 2); } textAlign(CENTER, CENTER); noStroke(); }
...
// drawKitty - ADDED Bow Drawing Logic!
function drawKitty(inCutscene = false) {
    if(!kitty) return;
    let drawBow = false;
    if (typeof getEquippedItem === 'function') { // Check if store function exists
    ...
    // Draw Jetpack FIRST
    if (kitty.hasJetpack) { /* ... same jetpack drawing ... */ rectMode(CENTER); fill(jetpackColor); stroke(50); strokeWeight(max(1, kitty.size * 0.03)); rect(kitty.x, kitty.y + kitty.size * 0.1, kitty.size * 0.6, kitty.size * 0.7, kitty.size * 0.1); rect(kitty.x - kitty.size * 0.2, kitty.y + kitty.size * 0.4, kitty.size * 0.15, kitty.size * 0.2); rect(kitty.x + kitty.size * 0.2, kitty.y + kitty.size * 0.4, kitty.size * 0.15, kitty.size * 0.2); if (!inCutscene || cutsceneStep < 3 || cutsceneStep > 4) { let flameSize = kitty.size * map(abs(kitty.bobOffset), 0, kitty.size * 0.05, 0.3, 0.6) * random(0.8, 1.2); fill(255, random(150, 200), 0, 200); noStroke(); triangle(kitty.x - kitty.size * 0.2, kitty.y + kitty.size * 0.5, kitty.x - kitty.size * 0.2 - flameSize * 0.3, kitty.y + kitty.size * 0.5 + flameSize, kitty.x - kitty.size * 0.2 + flameSize * 0.3, kitty.y + kitty.size * 0.5 + flameSize); triangle(kitty.x + kitty.size * 0.2, kitty.y + kitty.size * 0.5, kitty.x + kitty.size * 0.2 - flameSize * 0.3, kitty.y + kitty.size * 0.5 + flameSize, kitty.x + kitty.size * 0.2 + flameSize * 0.3, kitty.y + kitty.size * 0.5 + flameSize); } }

    // Draw Kitty Body
    fill(kittyColor); stroke(50); strokeWeight(max(1, kitty.size * 0.05)); rectMode(CENTER); rect(kitty.x, kitty.y, kitty.size, kitty.size);
    // Ears
    let earSize = kitty.size * 0.4; let earOffset = kitty.size * 0.1;
    triangle(kitty.x - kitty.size/2 + earOffset, kitty.y - kitty.size/2, kitty.x - kitty.size/2 + earOffset, kitty.y - kitty.size/2 - earSize, kitty.x - earOffset, kitty.y - kitty.size/2);
    triangle(kitty.x + kitty.size/2 - earOffset, kitty.y - kitty.size/2, kitty.x + kitty.size/2 - earOffset, kitty.y - kitty.size/2 - earSize, kitty.x + earOffset, kitty.y - kitty.size/2);
    // Tail
    ...
    if (drawBow) {
         let bowSize = kitty.size * 0.3;
         fill(255, 100, 150); // Cute pink bow color
         stroke(50); strokeWeight(1);
         // Position slightly above head, between ears
         let bowX = kitty.x;
         let bowY = kitty.y - kitty.size * 0.5; // Above head center
         // Simple bow shape using two triangles
         triangle(bowX - bowSize, bowY, bowX, bowY - bowSize * 0.4, bowX, bowY + bowSize * 0.4); // Left part
         triangle(bowX + bowSize, bowY, bowX, bowY - bowSize * 0.4, bowX, bowY + bowSize * 0.4); // Right part
    }
...
// --- Input Handling --- (UPDATED Store Logic) ---
function handlePressStart() {
    ...
    // Back Button Check (Endless Playing)
    if (gameState === 'playing' && isEndlessMode && backButton) { if (pressX >= backButton.x && pressX <= backButton.x + backButton.w && pressY >= backButton.y && pressY <= backButton.y + backButton.h) { /* ... save scores, reset ... */ console.log("Back button pressed!"); if (highestStreakInSession > endlessHighScore) { endlessHighScore = highestStreakInSession; try { localStorage.setItem('kittyEndlessHighScore', endlessHighScore); console.log("New Endless High Score Saved:", endlessHighScore); } catch(e){ console.warn("Failed to save endless high score:", e); } } try { localStorage.setItem('kittyTotalPlushies', totalPlushiesCollected); } catch(e){ console.warn("Failed to save total plushies:", e); } gameState = 'start'; resetGame(); return; } }

    // Start Screen Buttons
    if (gameState === 'start') {
        if (endlessModeButton && pressX >= endlessModeButton.x && pressX <= endlessModeButton.x + endlessModeButton.w && pressY >= endlessModeButton.y && pressY <= endlessModeButton.y + endlessModeButton.h) { isEndlessMode = !isEndlessMode; console.log("Endless Mode Toggled:", isEndlessMode); return; }
        if (storeButton && pressX >= storeButton.x && pressX <= storeButton.x + storeButton.w && pressY >= storeButton.y && pressY <= storeButton.y + storeButton.h) { console.log("Store button pressed! Entering store..."); gameState = 'store'; return; }
    }
    ...
    if (gameState === 'store') {
        if (typeof handleStoreInput === 'function') {
            ...
            if (typeof purchaseCost === 'number' && purchaseCost > 0) { // If bought successfully
                totalPlushiesCollected -= purchaseCost; // Subtract cost
            ...
            } else if (purchaseCost === true) {
                // Item handled (equip/unequip/back), do nothing else
            }
        } else { console.warn("handleStoreInput function not found! Switching back to start."); gameState = 'start'; }
        return; // Always return after store input check
    }
    ...
    else if (gameState === 'start') { kitty.hasJetpack = true; resetGame(); gameState = 'playing'; } // Should only happen if tap was outside buttons
...
// resetGame (Save total plushies)
function resetGame() {
    // Save total plushies whenever a game ends (before score reset)
     if (lastGameState === 'playing') { // Only save if coming from playing state
